...
import { comments, follows, postMedia, posts, profiles, postReactions } from '$lib/server/db/schema';
...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  const [post] = await db.select({ id: posts.id, caption: posts.caption, createdAt: posts.createdAt, authorName: profiles.realName, authorUsername: profiles.username, mediaId: postMedia.id })
    .from(posts).innerJoin(profiles, eq(profiles.userId, posts.authorId)).leftJoin(postMedia, eq(postMedia.postId, posts.id)).where(eq(posts.id, params.id)).limit(1);
