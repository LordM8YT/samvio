...
<main class="profile-page">
  <div class="profile-shell">
    <header class="profile-header">
      ...
      <div><h1>{data.profile.realName} {#if data.profile.role === 'admin'}<span class="role-badge">Admin</span>{:else if data.profile.role === 'moderator'}<span class="role-badge mod">Moderator</span>{/if}</h1><p>@{data.profile.username}</p>{#if data.profile.bio}<div class="bio">{data.profile.bio}</div>{/if}</div>
...
<style>
  ...
  .role-badge{display:inline-flex;vertical-align:middle;padding:4px 8px;border-radius:999px;background:#315d49;color:#fff;font:800 9px system-ui,sans-serif;letter-spacing:.05em;text-transform:uppercase}.role-badge.mod{background:#b76538}.profile-posts{display:grid;gap:18px}
</style>
