<script lang="ts">
  ...
  async function blockProfile() {
    if (!confirm(`Blokkere @${data.profile.username}? Dere fjernes som følgere og kan ikke se hverandres profiler.`)) return;
    const response = await fetch('/api/blocks', { method: data.isBlocked ? 'DELETE' : 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ targetId: data.profile.userId }) });
    if (response.ok) location.reload(); else safetyStatus = 'Kunne ikke oppdatere blokkeringen.';
  }
  async function reportProfile() {
    if (!confirm(`Sende profilen @${data.profile.username} til moderering?`)) return;
    const response = await fetch('/api/reports', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ targetType: 'user', targetId: data.profile.userId, reason: 'other', details: 'Profil rapportert fra profilsiden.' }) });
    safetyStatus = response.ok ? 'Profilen er sendt til moderering.' : 'Kunne ikke sende rapporten.';
  }
...
<main class="profile-page">
  <div class="profile-shell">
    ...
    <header class="profile-header">
      ...
      <div><h1>{data.profile.realName} {#if data.profile.role === 'admin'}<span class="role-badge">Admin</span>{:else if data.profile.role === 'moderator'}<span class="role-badge mod">Moderator</span>{/if} {#if data.profile.verified}<span class="verified-badge">{data.verificationProvider === 'vipps' ? 'Verifisert med Vipps' : 'Verifisert identitet'}</span>{/if}</h1><p>@{data.profile.username}</p></div>
      {#if data.isOwnProfile}
        <a class="edit-profile" href="/innstillinger#profil">Rediger profil</a>
      {:else}
        <div class="profile-controls">{#if !data.isBlocked}<form method="POST" action={data.followStatus === 'accepted' ? '?/unfollow' : '?/follow'}><button>{data.followStatus === 'accepted' ? 'Følger' : data.followStatus === 'pending' ? 'Forespurt' : 'Følg'}</button></form><button class="quiet" onclick={reportProfile}>Rapporter</button>{/if}<button class="quiet" onclick={blockProfile}>{data.isBlocked ? 'Opphev blokkering' : 'Blokker'}</button></div>
      {/if}
    </header>
    ...
    {#if safetyStatus}<p class="safety-status" role="status">{safetyStatus}</p>{/if}
    {#if activeTab === 'about'}
...
<style>
  ...
  .role-badge{display:inline-flex;vertical-align:middle;padding:4px 8px;border-radius:999px;background:#315d49;color:#fff;font:800 9px system-ui,sans-serif;letter-spacing:.05em;text-transform:uppercase}.role-badge.mod{background:#b76538}.verified-badge{display:inline-flex;vertical-align:middle;padding:4px 8px;border:1px solid #a8d6c0;border-radius:999px;background:#edf8f2;color:#245a43;font:800 9px system-ui,sans-serif;letter-spacing:.02em}.profile-posts{display:grid;gap:18px}.profile-overview{display:flex;align-items:center;gap:8px;margin:-12px 0 22px;padding:15px 17px;border:1px solid #ddd7cc;border-radius:13px;background:linear-gradient(120deg,#fff,#f1f6f3)}.profile-overview>div{min-width:82px;display:grid;text-align:center}.profile-overview strong{font:600 22px 'Newsreader',serif}.profile-overview span{color:#737873;font-size:10px}.profile-overview a{margin-left:auto;display:flex;align-items:center;gap:7px;padding:10px 13px;border-radius:9px;background:#315d49;color:#fff;font-size:12px;font-weight:700}.profile-tabs{display:flex;gap:22px;margin-bottom:18px;border-bottom:1px solid #ddd7cc}.profile-tabs a{padding:11px 2px;color:#747874;font-size:12px;font-weight:700}.profile-tabs a.active{margin-bottom:-1px;border-bottom:2px solid #315d49;color:#315d49}.about-profile{min-height:180px;margin-bottom:18px;padding:20px;border:1px solid #eadfce;border-radius:12px;background:#f8f2e8}.about-profile>p{margin:8px 0 0;color:#555;font-size:13px;line-height:1.55}.about-empty{min-height:125px;display:grid;place-items:center;align-content:center;gap:7px;color:#737873;text-align:center}.about-empty p{margin:0;font-size:13px}.about-empty a{color:#315d49;font-size:12px;font-weight:800;text-decoration:underline}@media(max-width:600px){.profile-overview{display:grid;grid-template-columns:repeat(3,1fr)}.profile-overview a{grid-column:1/-1;width:100%;justify-content:center;margin:4px 0 0}}
  .profile-controls{display:flex;align-items:center;gap:6px}.profile-controls form{display:flex}.profile-controls .quiet{border-color:#d7d4cd;background:#fff;color:#5f6561;font-size:10px}.safety-status{padding:10px 13px;border-radius:9px;background:#edf3ef;color:#315d49;font-size:12px}@media(max-width:600px){.profile-controls{grid-column:1/-1;display:grid;grid-template-columns:1fr 1fr}.profile-controls form{grid-column:1/-1}.profile-controls button{width:100%}}
  .profile-header{position:relative;z-index:2;grid-template-columns:minmax(0,1fr) auto;min-height:108px;margin:-1px 18px 28px;padding:20px 20px 18px 108px;border-radius:0 0 16px 16px}.profile-header .avatar{position:absolute;left:20px;top:-30px;width:76px;height:76px;border-width:3px;box-shadow:0 3px 12px #1b2a2226}.profile-header>div:not(.profile-controls){align-self:center;min-width:0}.profile-header h1{display:flex;align-items:center;flex-wrap:wrap;gap:7px;font-size:30px;line-height:1.05}.profile-header h1 .role-badge{margin:0}.profile-header .edit-profile,.profile-header .profile-controls{align-self:center}.profile-overview{margin-top:-12px}@media(max-width:600px){.profile-header{grid-template-columns:1fr;min-height:118px;margin:-1px 8px 22px;padding:45px 14px 14px}.profile-header .avatar{left:16px;top:-34px;width:68px;height:68px}.profile-header>div:not(.profile-controls){padding-left:80px}.profile-header h1{font-size:24px}.profile-header .edit-profile,.profile-header .profile-controls{grid-column:1;margin-top:10px}.profile-header .edit-profile{text-align:center}.profile-overview{margin-top:0}}
</style>
