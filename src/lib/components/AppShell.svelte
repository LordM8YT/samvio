...
<nav class="mobile-nav" aria-label="Mobilmeny">{#each mobileItems as item}<a class:active={active(item.href)} href={item.href} aria-label={item.label} title={item.label}><item.icon size={24} strokeWidth={active(item.href) ? 2.5 : 1.8}/></a>{/each}</nav>
