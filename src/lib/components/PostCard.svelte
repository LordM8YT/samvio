<script lang="ts">
  ...
  let { post } = $props<{ post: { id: string; caption: string | null; createdAt: Date; authorName: string; authorUsername: string; authorRole?: 'user' | 'moderator' | 'admin'; mediaId: string | null; liked?: boolean } }>();
...
<article class="post-card">
  <header><a class="post-avatar" href={`/bruker/${post.authorUsername}`} aria-label={`Se profilen til ${post.authorName}`}><UserRound size={19}/></a><div><a href={`/bruker/${post.authorUsername}`}><strong>{post.authorName}</strong>{#if post.authorRole === 'admin'}<span class="staff-badge">Admin</span>{:else if post.authorRole === 'moderator'}<span class="staff-badge mod">Moderator</span>{/if}<small>@{post.authorUsername}</small></a></div><time datetime={post.createdAt.toISOString()}>{post.createdAt.toLocaleDateString('nb-NO', { day: 'numeric', month: 'short' })}</time></header>
